defmodule LibsqlTest do
  ...
  @remote_host "libsql://testing.turso.io"
  @remote_token ""
  ...
  #   {:ok, %Libsql.Result{last_insert_id: 0}} = Libsql.Connection.query_blocking(conn, @drop_table, [])
  #   {:ok, %Libsql.Result{last_insert_id: 0}} = Libsql.Connection.query_blocking(conn, @create_table, [])
  ...
  test "new remote_replica db" do
    {:ok, db} = Libsql.Database.new_remote_replica("replicate.db", @remote_host, @remote_token)
    {:ok, conn} = Libsql.Database.connection(db)
    ...
    {:ok,
     %Libsql.Result{
       ...
       rows: [
         ...
         [2, "Cheese its", 2023, "PG-13", "120 min", "Cheese its everywhere!!", nil, nil, 1] | _
       ]
     }} = Libsql.Connection.query(conn, "select * from movies limit 100", [])
  end
end
